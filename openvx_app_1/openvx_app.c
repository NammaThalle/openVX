...
void openvx_app(int argc, char *argv[])
{
  ...
  image = vxCreateImage(context, IMAGE_WIDTH, IMAGE_HEIGHT, VX_DF_IMAGE_RGB);
  ...
  if (fp)
  {
    ...
    fclose(fp);
    printf("Created image.rgb\n");
  }
  else
  {
    printf("Error creating image.rgb\n");
  }
